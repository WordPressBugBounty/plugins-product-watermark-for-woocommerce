<?php
if ( ! class_exists( 'brwatermark_free_funnels' ) ) {
	class brwatermark_free_funnels {
		...
		public function spring_premium_days_2026(): void {
			$plugin_version_capability = apply_filters( 'brfr_get_plugin_version_capability_image_watermark', 0 );
			$type = ( $plugin_version_capability >= 15 ) ? 'paid' : 'free';

			$name       = 'spring_premium_days_2026';
			$media      = '?utm_source=' . $type . '_plugin&utm_medium=admin_notice&utm_campaign=spring_2026&utm_content=top_banner_cta&utm_term=watermarks';
			...
			if ( ! $is_closed and $start_time <= time() and $end_time > time() ) {
				...
				<div class="berocket-notice-template-big" style="">
					<div class="berocket-notice-description-container">
						...
						<h3>Protect your images and build a stronger brand identity</h3>
						<ul style="font-size: 16px">
							<li>Add text or image watermarks to secure your content</li>
							<li>Customize position, size, transparency, and styles</li>
							<li>Use multiple watermarks and apply them in bulk</li>
						</ul>
					...
					<div class="berocket-notice-actions-container">
						...
						<div class="berocket-notice-buttons-container">
							<a href="https://berocket.com/woocommerce-products-image-watermark/' . $media . '" 
							   class="button notice-action-link not_berocket_button" target="_blank" 
							   style="position: static; display: inline-block; right: 0; top: 0; margin: 0;font-size: 24px;padding-left: 25px;padding-right: 25px;">Upgrade Watermarks → 30% OFF</a>
